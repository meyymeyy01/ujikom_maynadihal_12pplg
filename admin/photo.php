...
<body>
  ...
  <main id="main" class="main">
    ...
    <?php
    $query = "SELECT foto.fotoID, foto.judulFoto, foto.deskripsiFoto, foto.tanggalUnggah, foto.lokasiFile, album.namaAlbum, foto.userId, user.username FROM foto INNER JOIN album ON foto.albumId = album.albumId INNER JOIN user ON foto.userId = user.userId ORDER BY foto.tanggalUnggah DESC";
      ...
      <div class="row align-items-top">
        <?php foreach ($images as $image): ?>
          ...
          <div class="col-md-3 mb-4"> <!-- Added margin-bottom for spacing -->
            <div class="card">
              <?php if (file_exists("../user/photo/" . $image['lokasiFile'])): ?>
                <img src="../user/photo/<?= $image['lokasiFile'] ?>" class="card-img-top" alt="<?= $image['deskripsiFoto'] ?>">
              <?php else: ?>
              ...
              <div class="card-body">
                ...
                <div style="font-size: small;">Oleh : <?= $image['username']; ?></div>
                <div style="font-size: small;">Tanggal : <?= $tglIndonesia; ?></div>
                <div style="font-size: small;">Album: <?= $image['namaAlbum'] ?></div>
              </div>
              <div class="card-footer">
              <form action="delete_photo.php" method="POST" onsubmit="return confirm('Anda yakin ingin menghapus foto ini?');">
